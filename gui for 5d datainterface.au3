...
While 1
	...
	Switch $nMsg
		...
		Case $b_addvariant
			_controller_addVariant($data, $multiverse)
			If @error Then
				$name = InputBox("missing name", "enter name")
				If @error Then
					MsgBox(16, "No name given", "No name given variant not added")
				EndIf
				_controller_addVariant($data, $multiverse)
			EndIf
			GUISetState(@SW_ENABLE)
